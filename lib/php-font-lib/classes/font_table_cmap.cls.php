			...
			*/
			switch ( $subtable['format'] ) {
				case 4:
					...
					break;
				case 12:
					$font->readUInt16(); // the rest of format 16/32 byte
					$subtable += $font->unpack(self::$subtable_v12_format);

					$glyphIndexArray = array();
					for($c = 0; $c <= $subtable['nGroups']; $c++) {
						$startCharCode = $font->readUInt32();
						$endCharCode = $font->readUInt32();
						$startGlyphCode = $font->readUInt32();
						if ( $startCharCode == $endCharCode ) {
							$glyphIndexArray[$endCharCode] = $startGlyphCode;
						} else {
							for ($_c = $startCharCode; $_c <= $endCharCode; $_c++) {
								$glyphIndexArray[$_c] = $startGlyphCode;
								$startGlyphCode++;
							}
						}
					}
